const quotes = [
  "ты не обязана заслуживать право на еду, отдых и заботу о себе.",
  "твой самый трудный день не отменяет весь путь, который ты уже прошла.",
  ...
  "тебе можно быть уставшей. тебе можно остановиться.",
  ...
  "еда не делает тебя хорошей или плохой. это еда, а не моральный тест."
];

const content = {
  care: {
    title: "бережно о себе",
    body: `
      <p class="note">это пространство не для контроля себя, а для того, чтобы немного снизить давление.</p>
      <h3>что здесь будет</h3>
      <ul>
        <li>что делать, когда усиливаются навязчивые мысли о еде и теле;</li>
        <li>как переживать сильный позыв к компенсаторному поведению;</li>
        <li>почему восстановление может идти неровно;</li>
        <li>как отличать заботу о здоровье от поведения РПП.</li>
      </ul>
  ...
  supplements: {
    title: "база БАДов",
    body: `
      <p class="note">карточки БАДов будут разделять заявления производителей и данные исследований.</p>
